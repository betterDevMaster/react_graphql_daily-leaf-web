...
if (isSuperAdmin()) {
  ...
  menuItems.push({title: 'All Deals', icon: 'now-ui-icons shopping_tag-content', link: '/admin/deals'})
}

const breadCrumbs = [
  {title: 'Home', url: '/admin'},
  {title: 'Deals', url: '/admin/deals'},
  {title: 'Archived', url: '/admin/deals/archived', active: true}
]
...
class Deals extends Component {
  render () {
    ...
    return (
      <Page title='Archived Deals' breadcrumbs={breadCrumbs} menuItems={menuItems} >
        <Card title='Deals'>
          { isSuperAdmin() ? <DataTable id='dealTable' ref={(datatable) => { this.datatable = datatable }} columns={dataColumns} source={`${server.url}/datatables/archived-deals`} /> : null }
        </Card>
      </Page>
    )
  }
